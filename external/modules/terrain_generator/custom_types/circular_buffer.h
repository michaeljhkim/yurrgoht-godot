...
/*
ORIGINAL CREATOR: vinitjames
ORIGINAL REPOSITORY: https://github.com/vinitjames/circularbuffer
*/
template<typename T>
...
typename CircularBuffer<T>::reference CircularBuffer<T>::front() {
	std::lock_guard<std::mutex> _lck(_mtx);
	if(empty()) {
		CRASH_NOW_MSG("front function called on empty buffer");
...
typename CircularBuffer<T>::reference CircularBuffer<T>::back() {
	std::lock_guard<std::mutex> _lck(_mtx);
	if(empty()) {
		CRASH_NOW_MSG("back function called on empty buffer");
...
void CircularBuffer<T>::pop_front(){
	std::lock_guard<std::mutex> _lck(_mtx);
	if(empty()) {
		ERR_FAIL_MSG("pop_front called on empty buffer");
...
typename CircularBuffer<T>::reference CircularBuffer<T>::operator[](size_t index) {
	...
	if((index<0)||(index>=_size)) {
		ERR_FAIL_MSG("Index is out of Range of buffer size");
