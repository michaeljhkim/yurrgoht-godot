...
vec3 get_decal(vec3 albedo, vec2 uv) {
	for (int i = 0; i < 3; ++i) {
		...
		if (Math::abs(decal_uv.x) > 0.499 || Math::abs(decal_uv.y) > 0.499) {
			continue;
		}
		float decal = 0.0;
		switch (i) {
	...
	if (_editor_decal_visible[0] && _editor_decal_size[0] <= _editor_crosshair_threshold) {
		...
		bool h = Math::abs(cross_uv.y) < line_thickness && Math::abs(cross_uv.x) < line_end && Math::abs(cross_uv.x) > line_start;
		bool v = Math::abs(cross_uv.x) < line_thickness && Math::abs(cross_uv.y) < line_end && Math::abs(cross_uv.y) > line_start;
