...
void Terrain3DAssets::destroy() {
	...
	if (_scenario.is_valid()) {
		/*
		RS::get_singleton()->free_rid(_mesh_instance);
		RS::get_singleton()->free_rid(_fill_light_instance);
		RS::get_singleton()->free_rid(_fill_light);
		RS::get_singleton()->free_rid(_key_light_instance);
		RS::get_singleton()->free_rid(_key_light);
		RS::get_singleton()->free_rid(_camera);
		RS::get_singleton()->free_rid(_viewport);
		RS::get_singleton()->free_rid(_scenario);
		*/
		RS::get_singleton()->call("free_rid", _mesh_instance);
		RS::get_singleton()->call("free_rid", _fill_light_instance);
		RS::get_singleton()->call("free_rid", _fill_light);
		RS::get_singleton()->call("free_rid", _key_light_instance);
		RS::get_singleton()->call("free_rid", _key_light);
		RS::get_singleton()->call("free_rid", _camera);
		RS::get_singleton()->call("free_rid", _viewport);
		RS::get_singleton()->call("free_rid", _scenario);
		_mesh_instance = RID();
...
void Terrain3DAssets::create_mesh_thumbnails(const int p_id, const Vector2i &p_size) {
	...
	for (int i = start; i < end; i++) {
		...
		//RS::get_singleton()->force_draw();
		RS::get_singleton()->call("force_draw");
