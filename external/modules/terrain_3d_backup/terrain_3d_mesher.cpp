	...
	for (int lod = 0; lod < _clipmap_rids.size(); lod++) {
		...
		for (int mesh = 0; mesh < lod_array.size(); mesh++) {
			...
			for (int instance = 0; instance < mesh_array.size(); instance++) {
				//RS::get_singleton()->free_rid(mesh_array[instance]);
				RS::get_singleton()->call("free_rid", mesh_array[instance]);
			}
			mesh_array.clear();
		}
		lod_array.clear();
...
void Terrain3DMesher::_clear_mesh_types() {
	...
	for (int m = 0; m < _mesh_rids.size(); m++) {
		//RS::get_singleton()->free_rid(_mesh_rids[m]);
		RS::get_singleton()->call("free_rid", _mesh_rids[m]);
	}
	_mesh_rids.clear();
	return;
}
	...
	for (int lod = 0; lod < _clipmap_rids.size(); ++lod) {
		...
		for (int mesh = 0; mesh < lod_array.size(); ++mesh) {
			...
			for (int instance = 0; instance < mesh_array.size(); ++instance) {
				...
				RS::get_singleton()->call("instance_reset_physics_interpolation", mesh_array[instance]);
			}
		}
	}
	return;
