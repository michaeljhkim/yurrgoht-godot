...
void Terrain3DData::save_directory(const String &p_dir) {
	...
	if (IS_EDITOR && !cast_to<EditorInterface>(EditorInterface::get_singleton())->get_resource_filesystem()->is_scanning()) {
		cast_to<EditorInterface>(EditorInterface::get_singleton())->get_resource_filesystem()->scan();
	}
}
